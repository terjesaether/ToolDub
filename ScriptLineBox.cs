...
namespace Nordubb.DubTool
{
  ...
  internal class ScriptLineBox : UserControl
  {
    ...
    private ScriptLine m_scriptline;
    ...
    public ScriptLineBox(ScriptLine sl)
    {
      m_scriptline = sl;
      InitializeControl();
      m_characterbox.Text = string.Format("{0}:", m_scriptline.Character);
      m_linebox.Text = m_scriptline.Line;
      m_timebox.Text = m_scriptline.Offset.ToString().Substring(3, 5);
    }
    ...
    public void RefreshOffset()
    {
      if (m_timebox.Focused)
        m_timebox.Text = m_scriptline.Offset.ToString().Substring(0, 11);
      else
        m_timebox.Text = m_scriptline.Offset.ToString().Substring(3, 5);
    }
    ...
    private void CharacterLeft(object sender, EventArgs e)
    {
      ...
      if (!(m_scriptline.Character != str))
        return;
      SetSaveNeeded();
      m_scriptline.Character = str;
    ...
    private void SaveFocus()
    {
      ...
      ((DubTool) Parent).SetEditState(state, m_scriptline);
    }
    ...
    private void TimeBoxLeft(object sender, EventArgs e)
    {
      ...
      {
        m_timebox.Width = (int) CreateGraphics().MeasureString(m_timebox.Text, m_timebox.Font, ParentForm.Width).Width + 5;
        if (!m_scriptline.Offset.Equals(zero))
        {
          SetSaveNeeded();
          m_scriptline.Offset = zero;
          SaveFocus();
          SortScript();
        }
        else
    ...
    {
      if (!(m_scriptline.Line != m_linebox.Text))
        return;
      SetSaveNeeded();
      m_scriptline.Line = m_linebox.Text;
    }
